    ...
    for addr_lower, sig in addr_map.items():
        ...
        if new_status != old_status:
            ...
            if update_signal_status(sig['id'], new_status):
                ...
                # KIRIM alert message terpisah supaya dapat notification
                alert_msgs = {
                    "HIT_TP1": f"🟢 <b>{sig['token_name']}</b> HIT TP1!\nClose 50% at ${sig['tp1']:.8f}",
                    "HIT_TP2": f"🟢 <b>{sig['token_name']}</b> HIT TP2!\nClose 30% at ${sig['tp2']:.8f}",
                    "CLOSED_TP3": f"🏁 <b>{sig['token_name']}</b> TP3 CLOSED!\nFinal close at ${sig['tp3']:.8f}",
                    "CLOSED_SL": f"🛑 <b>{sig['token_name']}</b> SL HIT\nStop loss at ${sig['sl']:.8f}",
                }
                if new_status in alert_msgs:
                    send_telegram_message(alert_msgs[new_status])
