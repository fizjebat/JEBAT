"""
==========================================================================
PROJECT JEBAT v6.0 — Precision Sniper
==========================================================================
Perubahan dari v5:
  ✅ Universe: new_pools + trending merged (discovery awal)
  ✅ h1 min gate: ≥8% (bukan sekadar >0%)
  ✅ h24 cap: ≤250% (block exhausted pumps)
  ✅ Min tx count: ≥30 transaksi h1
  ✅ Volume spike detector (h1/h6 ratio)
  ✅ CEX guard via CoinGecko (block listed tokens)
  ✅ Tighter momentum scoring + early-mover bonus
  ✅ Age max: 120h (dari 168h)
  ✅ Semua v5 features kekal
==========================================================================
"""
import os
import time
...
AGE_MIN_HOURS = 2
AGE_MAX_HOURS = 120             # v6: 5 hari (dari 168h) — CEX guard handle yang lama

# v6 new filter thresholds
H1_MIN_CHANGE    = 8.0          # Minimum h1 % — naik dari >0 ke ≥8%
H24_CHANGE_MAX   = 250.0        # Cap h24 — dah pump >250% = late/distribution
MIN_H1_TX        = 30           # Minimum tx h1 — data tak reliable kalau sikit
...
# 🛡️ CEX GUARD (v6 NEW)
# ==========================================
_COINGECKO_CHAIN_MAP = {'solana': 'solana', 'bsc': 'binance-smart-chain'}
_MAJOR_CEX_IDS = {
    'binance', 'okex', 'kucoin', 'bybit_spot', 'gate', 'mexc',
    'bitget', 'huobi', 'htx', 'coinbase', 'crypto_com'
}
_cex_cache = {}  # {addr_key: {'result': bool, 'ts': datetime}}

def is_on_cex(chain, token_address):
    """
    Check CoinGecko — kalau token ada tickers pada major CEX, block.
    Cache 12 jam per address. Fail-open on error/429.
    """
    ...
    if cached:
        ...
        if age_secs < 43200:  # 12 jam cache
    ...
    try:
        ...
        if resp.status_code == 429:
            logging.info(f"[CEX] CoinGecko rate-limited — skip check, fail-open")
            return False
        ...
        if mcap_rank and mcap_rank < 1500:
            ...
            logging.info(f"[CEX] BLOCKED: rank #{mcap_rank}")
            return True

        tickers = data.get('tickers', [])[:30]
        for t in tickers:
            market_id = t.get('market', {}).get('identifier', '').lower()
            if market_id in _MAJOR_CEX_IDS:
                _cex_cache[cache_key] = {'result': True, 'ts': datetime.now(timezone.utc)}
                logging.info(f"[CEX] BLOCKED: listed on {market_id}")
                return True
        ...
        logging.warning(f"CEX guard error: {e}")
        return False

# ==========================================
# 📊 VOLUME SPIKE DETECTOR (v6 NEW)
# ==========================================
def is_volume_spike(attrs):
    """h1 vol > 80% of h6 vol = whale spike, bukan organic demand."""
    vol_h1 = float(attrs.get('volume_usd', {}).get('h1', 0) or 0)
    vol_h6 = float(attrs.get('volume_usd', {}).get('h6', 0) or 0)
    if vol_h6 <= 0 or vol_h1 <= 0:
        return False
    return (vol_h1 / vol_h6) > 0.80

# ==========================================
# 🆕 NEW POOLS FETCHER (v6 NEW)
# ==========================================
def fetch_new_pools(chain, max_retries=3):
    """Ambil pool BARU — earlier discovery = better edge than trending."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 200:
                ...
                report_error('WARNING', 'geckoterminal', f'new_pools {chain}: {resp.status_code}')
                return []
        except Exception as e:
            if attempt < max_retries - 1:
                time.sleep(5 * (attempt + 1))
                continue
            report_error('CRITICAL', 'geckoterminal', f'fetch_new_pools {chain} failed', str(e))
            return []
    return []
...
def score_momentum(attrs):
    """
    0-25 markah — v6: tighter thresholds + early-mover bonus.
    """
    ...
    # m5 freshness — masih bergerak SEKARANG
    if m5 >= 3: score += 7
    elif m5 >= 1: score += 5
    elif m5 >= 0: score += 2
    
    # Early-mover bonus: h1 kuat tapi h24 masih rendah = awal perjalanan
    if h1 >= 10 and h24 < 50:
        score += 5
...
def process_pool_candidate(pool, chain, rejected):
    """
    v6 filter sequence (cheapest → expensive):
    1. Age → 2. Liq → 3. Vol24 → 4. FDV → 5. h1 ≥8% → 6. h24 cap
    → 7. Min tx → 8. Sell pressure → 9. Buy vol → 10. Vol spike
    → 11. Address → 12. Cooldown → 13. Audit → 14. Score
    → 15. CEX guard (LAST — paling expensive, hanya qualified tokens)
    """
    ...
    # 5. [v6] h1 minimum — ≥8% bukan sekadar >0%
    ...
    # 6. [v6] h24 cap — block exhausted pumps
    ...
    # 7. [v6] Minimum transaction count
    h1_tx = attrs.get('transactions', {}).get('h1', {})
    ...
    # 15. [v6] CEX guard — LAST sebab paling expensive (CoinGecko API)
    #     Hanya token yang dah lulus semua filter akan sampai sini
    #     ~1-3 calls per scan cycle instead of ~40
...
def job_scan_market():
    ...
    for chain in TARGET_CHAINS:
        # [v6] Merge new_pools (discovery) + trending_pools (confirmation)
        new_pools = fetch_new_pools(chain)
        trending_pools = fetch_trending_pools(chain)
        
        seen_ids = set()
        merged = []
        for p in (new_pools + trending_pools):  # new first = priority
            pid = p.get('id', '')
        ...
        print(f"[JEBAT] 📡 {chain.upper()}: {len(merged)} pools ({len(new_pools)} new + {len(trending_pools)} trending)", flush=True)
...
def handle_bot_command(update):
    ...
    global H1_MIN_CHANGE, H24_CHANGE_MAX
        ...
        # Handle callback buttons
            ...
            if callback_data.startswith('set_'):
                ...
                if new_mode in SETTING_PRESETS:
                    ...
                    H1_MIN_CHANGE        = p.get('H1_MIN_CHANGE', 8.0)
                    H24_CHANGE_MAX       = p.get('H24_CHANGE_MAX', 250.0)
...
def start_scheduler():
    ...
    try:
        ...
        send_admin_log(
            f"🟢 <b>JEBAT v6.0 Booted</b>\n"
            f"Instance: <code>{INSTANCE_ID}</code>\n"
            f"Scanner | Watchlist | Monitor: ACTIVE\n"
            f"<i>CEX guard, vol spike, new_pools merge, tighter momentum.</i>"
        )
        return True
    except Exception as e:
...
@app.route('/')
def home():
    return f"JEBAT v6.0 — Instance {INSTANCE_ID}"
