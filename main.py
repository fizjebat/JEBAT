...
def format_signal_text(sig_data, status="ACTIVE", score_breakdown=None, audit_msg=""):
    ...
    return (
        ...
        f"{score_block}"
        f"🛡️ {audit_msg}\n"
        f"📊 Buys/Sells (h1): {sig_data.get('buy_sell_ratio', 'N/A')}\n"
...
def process_pool_candidate(pool, chain):
    ...
    # HARD KILL: Sell pressure check (cheap — no API call needed)
    # Kalau sells > 1.8x buys, ini distribution — insiders tengah dump
    h1_tx = attrs.get('transactions', {}).get('h1', {})
    h1_buys = int(h1_tx.get('buys', 0) or 0)
    h1_sells = int(h1_tx.get('sells', 0) or 0)
    if h1_buys > 0 and h1_sells / h1_buys > 1.8:
        return None  # Distribution detected — skip
    
    # HARD KILL: Buy volume vs Sell volume (dollar-weighted)
    # Kalau sell volume > buy volume, orang keluar lebih banyak wang
    ...
    if buy_vol > 0 and sell_vol > 0 and sell_vol / buy_vol > 1.5:
        return None  # More $ leaving than entering
    ...
    # HARD GATE: Momentum minimum 15/25 untuk FAST signal
    # Token dengan momentum lemah (<15) hanya layak watchlist, bukan instant signal
    # Ini elak signal yang lepas hanya sebab faktor lain compensate
    ...
    if h24_change > 100 or weak_momentum:
        sig_type = 'PULLBACK'  # Dah pump besar ATAU momentum lemah → watchlist
    else:
        sig_type = 'FAST'
...
def start_scheduler():
    ...
    # Heartbeat untuk refresh lock setiap 5 minit (bukan 1 — kurangkan Supabase load)
    scheduler.add_job(acquire_scheduler_lock, 'interval', minutes=5)
