...
def fetch_trending_pools(chain, max_retries=3):
    """
    Guna new_pools — token yang baru wujud (2-48 jam).
    Tujuan: tangkap token SEBELUM pump, bukan selepas.
    Cari accumulation: buy pressure tinggi, harga belum terbang.
    """
...
def score_momentum(attrs):
    """
    0-25 markah untuk ACCUMULATION pattern.
    Reward: buy pressure tinggi + harga belum terbang (sweet spot 2-12%).
    Penalise: token dah pump tinggi (>15%) = masuk lewat.
    """
    ...
    # Sweet spot: h1 antara 2-12% — ada movement tapi belum terbang
    if 5 <= h1 <= 12:  score += 12   # Ideal accumulation zone
    elif 2 <= h1 < 5:  score += 8    # Baru mula
    elif 0 < h1 < 2:   score += 5    # Sangat awal
    elif 12 < h1 <= 20: score += 4   # Dah bergerak, masih ok
    # h1 > 20: sudah ditapis sebelum ini (already_pumped filter)

    # h6 context: uptrend muda = terbaik
    if 0 < h6 <= 15:   score += 8    # Tren naik muda
    elif h6 > 15:      score += 4    # Dah naik banyak dalam 6j
    elif h6 >= -5:     score += 3    # Flat/slight dip — ok
    # h6 < -5: momentum lemah

    # m5: fresh activity dalam 5 minit
    if m5 >= 1:  score += 5
    elif m5 >= 0: score += 3
    elif m5 >= -2: score += 1
...
def calculate_realized_roi(status, r_pct, prev_status=None):
    """
    ROI calculation dengan 3 jenis SL:
    - CLOSED_SL selepas tiada TP  = SL_FULL  → -1R (loss sebenar)
    - CLOSED_SL selepas HIT_TP1   = SL_BE    → +TP1 portion sahaja (tiada kerugian)
    - CLOSED_SL selepas HIT_TP2   = SL_TP1   → TP1+TP2 portion (untung terkunci)
    """
    if status == 'CLOSED_SL':
        if prev_status == 'HIT_TP2':
            # SL trail ke TP1 — TP1+TP2 dah secured
            return (SCALE_TP1 * RR_TP1 + SCALE_TP2 * RR_TP2) * r_pct
        elif prev_status == 'HIT_TP1':
            # SL di BE — TP1 dah secured, baki 0
            return SCALE_TP1 * RR_TP1 * r_pct
        else:
            # SL_FULL — full loss
            return -r_pct
    if status == 'HIT_TP1':
...
def update_signal_status(signal_id, new_status, prev_status=None):
...
def process_pool_candidate(pool, chain, rejected):
    ...
    h1_total_tx = h1_buys + h1_sells

    # ACCUMULATION FILTER — masuk SEBELUM pump, bukan selepas
    # Token yang dah pump > 20% dalam h1: sudah lewat, skip
    if h1_change > 20:
        rejected['already_pumped'] = rejected.get('already_pumped', 0) + 1
        return None

    # Token yang dump teruk > -10%: trend negatif, skip
    if h1_change < -10:
        rejected['h1_dumping'] = rejected.get('h1_dumping', 0) + 1
        return None

    # Kalau ada data transaksi, mesti ada buy pressure (>52%)
    # Ini detect accumulation: ramai beli tapi harga belum naik banyak
    if h1_total_tx >= 20:
        buy_ratio = h1_buys / h1_total_tx
        if buy_ratio < 0.52:
    ...
    # ④ Rolling win rate penalty
    total_score -= rolling_wr_penalty
...
def job_scan_market():
    ...
    # ④ Rolling win rate — semak 20 signal terakhir
    last_20 = supabase_select(
        "signals", "status",
        "status=neq.ACTIVE&order=created_at.desc&limit=20"
    )
    if len(last_20) >= 10:  # Minimum 10 untuk valid
        wins_20 = sum(1 for s in last_20 if s['status'] in ['CLOSED_TP3','HIT_TP2','HIT_TP1'])
        sl_full_20 = sum(1 for s in last_20 if s['status'] == 'CLOSED_SL' and not s.get('prev_status','') in ['HIT_TP1','HIT_TP2'])
        closed_20 = wins_20 + sl_full_20
        wr_20 = (wins_20 / closed_20 * 100) if closed_20 > 0 else 50
        if wr_20 < 22:
            rolling_wr_penalty = 15
            print(f"[JEBAT] ⚠️ Rolling WR {wr_20:.1f}% < 22% — +15 penalti aktif", flush=True)
        else:
            rolling_wr_penalty = 0
...
def job_monitor_signals():
    ...
    # ③ Sweep recovery check — semak token CLOSED_SL yang perlu di-verify
    now_iso = datetime.now(timezone.utc).isoformat().replace('+00:00','Z')
    sweep_candidates = supabase_select(
        "signals", "id,token_address,token_name,sweep_check_at,sweep_sl_level",
        f"status=eq.CLOSED_SL&sweep_check_at=lt.{now_iso}&sweep_sl_level=gt.0"
    )
    if sweep_candidates:
        sweep_addrs = [s['token_address'] for s in sweep_candidates]
        sweep_prices = fetch_current_prices(sweep_addrs)
        for sc in sweep_candidates:
            addr = sc['token_address'].lower()
    ...
    for addr_lower, sig in addr_map.items():
        ...
        if new_status != old_status:
            # Update DB DULU — simpan prev_status untuk journal SL classification
            if update_signal_status(sig['id'], new_status, prev_status=old_status):
...
def generate_journal():
    ...
    # Fetch semua signals dengan prev_status untuk SL classification
    all_signals_full = supabase_select(
        "signals", "id,status,r_value_pct,chain,token_name,created_at,prev_status",
        f"created_at=gt.{cutoff}"
    )

    for sig in sorted(all_signals_full, key=lambda x: x.get('created_at', ''), reverse=False):
        status = sig['status']
        prev_st = sig.get('prev_status') or ''
        r_pct = float(sig.get('r_value_pct', 0) or 0)
        roi = calculate_realized_roi(status, r_pct, prev_st) if status != 'ACTIVE' else None
        roi_str = f"{roi:+.1f}%" if roi is not None else "—"

        # Label SL dengan betul
        if status == 'CLOSED_SL':
            if prev_st == 'HIT_TP2':
                sl_label = '🛡️ SL@TP1'   # untung terkunci
            elif prev_st == 'HIT_TP1':
                sl_label = '🛡️ SL@BE'    # tiada kerugian
            else:
                sl_label = '🛑 SL'        # full loss
        else:
            sl_label = status_labels.get(status, status)

        # Win/loss classification yang betul
        if status == 'CLOSED_SL' and prev_st not in ['HIT_TP1', 'HIT_TP2']:
            losses += 1
            total_roi += roi
        elif status == 'CLOSED_SL' and prev_st in ['HIT_TP1', 'HIT_TP2']:
            wins += 1  # SL_BE dan SL_TP1 dikira win (tiada kerugian/masih untung)
            total_roi += roi
        elif status in ['HIT_TP1', 'HIT_TP2', 'CLOSED_TP3']:
            wins += 1
            total_roi += roi
        ...
        raw_date = sig.get('created_at', '')[:10]  # "2026-05-28"
        try:
            y, m, d = raw_date.split('-')
            created = f"{int(d)}/{int(m)}/{y[2:]}"  # "28/5/26"
        except Exception:
            created = raw_date
        signal_lines.append(
            f"{ce} <b>{sig['token_name']}</b>  {sl_label}  {roi_str}  <i>{created}</i>"
        )
    ...
    # Hantar dalam chunks — setiap chunk max 3800 char (buffer untuk HTML tags)
    CHUNK_LIMIT = 3800
    ...
    # Chunk signal_lines supaya setiap message < CHUNK_LIMIT
    chunks = []
    ...
    # Hantar semua chunks, footer pada message terakhir
    for i, chunk in enumerate(chunks):
        if i == len(chunks) - 1:
            msg = chunk + footer
            if len(msg) > CHUNK_LIMIT:
